...
Run:  python notebooks/build/build_notebook.py        (writes notebooks/research/)
"""
import json, re, uuid, textwrap, os, sys
...
def slice_module(path):
    ...
    txt = open(os.path.join(SRC, path), encoding="utf-8").read()
...
# The repository root when this notebook runs inside the repository (it lives in notebooks/research/); elsewhere, e.g.
# in Colab, the current folder. Results are read from and written to REPO_ROOT/results.
REPO_ROOT = next((p for p in (".", "..", os.path.join("..", "..")) if os.path.isdir(os.path.join(p, "results"))
                  and os.path.isdir(os.path.join(p, "src"))), ".")
...
CFG = dict(
    ...
    results_dir=os.environ.get("QI_RESULTS_DIR", os.path.join(REPO_ROOT, "results")),
    **_presets[MODE],
)
...
_h5p = os.path.join(REPO_ROOT, "results", "h5_test", "records.csv")
if os.path.exists(_h5p):
    ...
    print("results/h5_test/records.csv not found (run `python experiments/exp_h5_cxm.py tune test analyze` in the repository)")
""")
md(r"""
...
_h7p = os.path.join(REPO_ROOT, "results", "h7_test", "records.csv")
if os.path.exists(_h7p):
    ...
    print("results/h7_test/records.csv not found (run `python experiments/exp_h7_swarm_seed.py tune test analyze` in the repository)")
""")
md(r"""
...
""")
code(r"""
_h6p = os.path.join(REPO_ROOT, "results", "h6_dynamic", "records.csv")
if os.path.exists(_h6p):
    ...
    print("results/h6_dynamic/records.csv not found (run `python experiments/exp_h6_dynamic.py run analyze` in the repository)")
""")
md(r"""
...
for _exp, _lab in [("h8_migration", "H8"), ("h10_elite_migration", "H10"), ("h11_event_elite", "H11"), ("h12_incremental_elite", "H12"),
                   ("h13_event_gamma", "H13")]:
    _p = os.path.join(REPO_ROOT, "results", _exp, "records.csv")
    if not os.path.exists(_p):
        print(f"{_p} not found (run the corresponding experiments/exp_*.py in the repository)"); continue
    _d = pd.read_csv(_p)
...
for fname in ("quantum_inspired_cloud_scheduler.ipynb", "quantum_inspired_MRFO_cloud_scheduler.ipynb"):   # identical copies
    os.makedirs(os.path.join(ROOT, "notebooks", "research"), exist_ok=True)
    out = os.path.join(ROOT, "notebooks", "research", fname)
