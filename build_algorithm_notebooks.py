...
Run:  python build_algorithm_notebooks.py
"""
...
def _definitions():
    ...
    defs = {}
    for mod in MODULES:
        txt = open(os.path.join(HERE, mod), encoding="utf-8").read()
...
def imports_block():
    txt = open(os.path.join(HERE, "qi_core.py"), encoding="utf-8").read()
...
""")
    ...
    nb.save(os.path.join(HERE, a["FILE"]))
