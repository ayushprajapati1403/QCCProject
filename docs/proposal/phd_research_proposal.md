...
The proposed work follows an implementation-first research loop that has already completed five versions (V1–V5) of the cycle observe → hypothesis → implement → run → ablate → adversarial test → prior-art recheck. Version 5 added eleven pre-registered hypotheses (H5–H15), each tested on held-out problem instances that were never used for any design choice. Everything is documented in runnable notebooks, a lab log, a research plan with every pre-registration and its outcome, and a report.

Findings of the preparatory work (20 000 evaluations per run; 30 seeds on seven instances in V4; in V5, 80 fresh held-out instances per static test and 60 scenario-seed pairs per dynamic test):

* Failure mode identified: with the rounding encoding, DMO and MRFO change 46–49 of 100 task assignments per candidate. Their gap to the makespan lower bound at 100 tasks is 75 % (DMO) and 84 % (MRFO), versus 1.4 % for a discrete GA at the same budget. The failure persists at scale: at 500–5 000 tasks their makespans stay 3.1–4.3 times the lower bound.

* Mechanism: the measurement-based register representation repairs both hosts. In the 30-seed confirmation QI-MRFO reaches a 1.1 % gap at 100 tasks and 2.8 % at 200 tasks on 20 VMs. It is better than MRFO on every instance (Holm-corrected p < 10⁻⁴) and GA-class: at most 0.64 percentage points behind a discrete GA on any of the seven instances, ahead on the other five, and significantly better on one. QI-DMO reaches 4.0 % (1.5 % once DMO's unconditional move is made greedy) and beats DMO on all 80 fresh held-out instances. At 500–5 000 tasks both quantum-inspired versions beat their originals in every run.

* Move types (V5): the product-state measurement changes tasks independently, so it almost never produces the correlated two-task exchange that nearly balanced schedules need. A correlated exchange measurement (a swap move on the busiest VM) lowers QI-MRFO's gap by 2.86 percentage points on 76 of 80 held-out instances (pre-registered H5) and beats the Max-Min heuristic on six of eight held-out families. Started from the Max-Min schedule, it ends within 0.03–0.22 % of the lower bound at 500–5 000 tasks. The same move does not transfer cleanly to QI-DMO, whose unconditional phase keeps worsening swaps (H14, H15): the value of a move depends on the host's acceptance rule.

* Re-scheduling under change with a migration price (V5): carrying the register state beats restarting on every change type. The representation's structural rules are VM removal as column deletion, VM addition as a uniform share and new tasks as uniform registers. Combined with an event-aware elite, incremental placement of new tasks and no noise floor after local changes, they give a configuration that beats the best list-heuristic strategy at every migration price tested: 58/2, 57/3 and 59/1 scenario-seed pairs on fresh seeds (H11–H13).

* Honest negatives:
  * The specifically quantum ingredients (amplitude squaring, signed interference) are irrelevant or slightly harmful. A probability-vector twin performs the same without the swap move and slightly better with it (68 of 80 held-out instances).
  * For one-time (static) makespan, a single-trajectory (1+1)-EA with the same moves is at least as good as the swarm (H7). The static gain belongs to the moves and the Max-Min start, not to the swarm.
  * Uniform "decoherence shocks" after a workload change never beat plain continuation at any churn severity from 10 % to 80 %.
  * A purity-regulated decoherence controller was falsified (H9). Once the swap move exists, the fixed noise floor no longer affects the static gap for c between 0 and 2 (development set).

* Prior art: the classical twin is essentially ICPSO (Strasser et al., GECCO 2016) with a new host; the floor is the EDA "margin" / Hε gate under another name. What is not found anywhere is a purity-calibrated per-candidate floor with a measured dose–response, VM removal as projective column deletion with warm re-optimisation, correlated exchange measurements in a register swarm, and any of this in a cloud scheduler.

The PhD builds on this base along three lines:

(a) Representation and theory. Formalise the register swarm as a per-individual univariate probabilistic model driven by swarm difference vectors and extended with correlated (exchange) measurements. Derive the expected move size $n - \sum_t \pi_t$ and its evolution under attraction, mixing and the depolarising channel. Obtain drift and runtime results (in the style of the chance-constrained makespan analyses of Neumann and co-workers and the r-valued EDA drift results of Doerr and Krejca) that predict when the noise floor matters and when exchange moves are needed, as a function of $n$, $m$, selection pressure and the acceptance rule. Validate the predictions against the measured dose–response and move-type diagnostics.

(b) Re-scheduling under change. Build on the V5 result that the event-aware configuration beats the best list-heuristic strategy under migration pricing. Replace its fixed event rules by severity- and cost-aware decisions: when to repair and when to re-optimise, which tasks to release, and how much noise to allow. Build a dynamic benchmark suite with trace-driven change generators (arrivals, failures, scale-out, drift). Evaluate against restart, continuation, hypermutation, random immigrants and learning-augmented/online baselines, and against a single-trajectory local search with the same moves, the control V5 showed to be necessary. Characterise the severity boundary beyond which carried state stops helping.
...
Expected outputs: a first paper on the failure mode of rounding encodings, its repair and the move-type diagnosis (workshop/CCGrid-level), a second on migration-priced re-optimisation with a measurement-based representation (journal: FGCS/TPDS-level target), a theory paper on move size and drift with a noise floor (GECCO/PPSN), a systems paper on the Kubernetes prototype, an open-source MEALPY-compatible implementation, and the thesis.
...
* Statistical protocol: every hypothesis pre-registered before it is run, with acceptance and falsification criteria; 30+ seeds or instance-level tests on held-out instance families (and real traces); paired non-parametric tests with Holm correction, effect sizes (Cliff's delta, Vargha–Delaney A12) and bootstrap confidence intervals; write-once experiment directories that record the code version; evaluation-budget fairness enforced in code (each measurement is one evaluation).

* Baselines: the algorithms the field uses (floor-encoded swarms), the algorithms it should use (discrete GA, ICPSO, PBIL/EDA, list heuristics, HEFT, and a single-trajectory local search with the same moves), and for dynamic settings restart/continue/hypermutation/random immigrants and learning-augmented online baselines.
...
* Reproducibility: all code, instances, seeds, logs and literature maps are released. The preparatory package includes 269 automated tests: objective versus brute force, Born-rule measurement, budget accounting, determinism, bit-exact golden fingerprints of earlier versions, notebook/module synchronisation and exact reproduction of committed records. It also includes Docker environments and Google Colab notebooks that save every run.
...
* A measurement-based register representation with correlated (exchange) measurements and structural rules for capacity changes. It makes swarm optimisers competitive with discrete GAs at equal budgets and, for re-optimisation under a migration price, better than the best list-heuristic strategy. It comes with a theory of move size and of when the noise floor matters.

* A characterisation of re-scheduling under change: which changes benefit from carried state, how the benefit decays with severity, and when to repair rather than re-optimise. The preparatory work already shows that removing the noise floor after local changes pays at medium and high migration prices (H13). It comes with a dynamic cloud-scheduling benchmark suite.

* A negative-result contribution that matters for the community. Quantum-specific ingredients (Born rule, interference) and uniform forgetting add nothing measurable on this problem class. For static makespan, a single-trajectory local search with the same moves matches the swarm. This sets a standard of evidence for "quantum-inspired" schedulers.
...
* Implementation-first preparatory work (2026):
  * a runnable research notebook (18 sections: problem model, faithful PSO/DMO/MRFO/GA implementations, the quantum-inspired mechanism, validation tests, baseline, convergence, statistics, ablation, sensitivity, adversarial cases, dynamic workloads, interpretation), executed end-to-end in Docker;
  * standalone Google Colab notebooks for QI-DMO and QI-MRFO;
  * a lab log with the full version history (V0–V5);
  * a research plan with eleven pre-registered, held-out hypothesis tests (H5–H15) and their outcomes, and a scaling study to 5 000 tasks;
  * a research report; live-web literature maps of quantum-inspired PSO (23 papers), DMO (5 quantum variants, 9 cloud applications), MRFO (6 quantum items, 12 cloud applications), quantum-inspired cloud scheduling on other hosts (19 papers) and the classical equivalents of every quantum mechanism (rotation gate = PBIL, tunnelling = Lévy/Cauchy mutation, decoherence = mutation shift / hypermutation, superposition + measurement = univariate EDA, purity floor = EDA margins); and a prior-art recheck of the exact implemented mechanism (ICPSO, QIEA/Hε gate, PBIL hypermutation, ACO pheromone equalisation, qudit-inspired optimisation).
...
* Months 3–6: replication of the held-out results on public traces (GoCJ, HPC2N, Google); energy/cost/deadline objectives, where list heuristics no longer dominate. Milestone: static-scheduling results paper.
* Months 6–10: severity- and cost-aware re-optimisation under change (repair versus re-optimise, beyond the event-aware rules of the preparatory work); dynamic benchmark suite and trace-driven change generators. Milestone: dynamic re-optimisation paper.
...
-----------------------------------------------------------------------------
Enclosures (paths in the project repository): docs/report/quantum_inspired_research_report.pdf, notebooks/research/quantum_inspired_cloud_scheduler.ipynb, notebooks/colab/ (QI-DMO and QI-MRFO notebooks), README.md, docs/lab_log.md, docs/research_plan_v5.md, docs/literature/.
